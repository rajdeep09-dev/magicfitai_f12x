...
-- 2. Drop policies that caused duplication or issues
DO $$ 
DECLARE pol RECORD;
BEGIN
...
-- 3. Consolidated Security Policies (Admin/Editor/Client)
-- Profiles
...
-- Creators (Include Client Update for approvals)
...
CREATE POLICY "Clients update approvals" ON creators FOR UPDATE USING (public.get_user_role() = 'client') WITH CHECK (public.get_user_role() = 'client');
...
CREATE POLICY "Clients read events" ON campaign_events FOR SELECT USING (true);
